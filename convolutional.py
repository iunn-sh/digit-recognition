...
def get_labels(filename, num_images):
  labels = pd.read_csv(path.join("./", filename),header=None)
  labels_header = labels.iloc[:,0:1].values
  labels = labels[[1]].values.ravel()  
  return labels_header , labels
...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  print('test_head({0[0]},{0[1]})'.format(test_header.shape))
